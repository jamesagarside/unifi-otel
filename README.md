...
| `linkcheck` **multi-line JSON** | **Parsed** ([#9](https://github.com/jamesagarside/unifi-otel/issues/9), [#34](https://github.com/jamesagarside/unifi-otel/issues/34)). A `recombine` operator reassembles the datagrams of one pretty-printed frame into a single record, and the payload becomes `unifi.speedtest`. Both framings are handled: continuation datagrams with no header, and continuation datagrams that re-emit the whole header — the second broke the original fix silently, shredding a payload into one record per line and gluing successive runs together. Both framings are **real captured frames**. Note that on current firmware the payload itself no longer parses into `unifi.speedtest.*` fields — that is [#36](https://github.com/jamesagarside/unifi-otel/issues/36), a separate gap the goldens now document. The residual log noise and the multi-gateway caveat are in [`docs/known-issues.md`](docs/known-issues.md). |
